...
mysql5.7没有默认的my.cnf,google后发现

![image.png](https://cdn.nlark.com/yuque/0/2020/png/282623/1585474998302-68992bd7-13b8-4d1f-9b8c-070e6fe24c20.png)
...
1. 把主库上的数据更改记录到二进制日志（Binary Log)中。（二进制日志事件）
2. 备库将主库上的日志复制到自己的中继日志（Relay Log）
3.  备库读取中继日志的事件，放到备库数据中。



![image.png](https://cdn.nlark.com/yuque/0/2020/png/282623/1585483331691-24832c91-cf91-4092-ac97-d5df9a2539ef.png)



### 复制原理

语句复制：主库记录会修改数据的sql，备库复制的时候是把sql重新执行一遍。

好处：简单，复制时传输的数据量小。

缺点：更新必须串行，否则sql执行顺序不同的结果不一样。sql执行的时候可能依赖当前时间戳，也会造成时间不一致。



行复制可以正确的复制每一行。缺点是有可能复制的代价很高。比如说一条sql进行了全表更新。
...
该策略是最成功且最通用的方法。它将数据分割成一小片，然后存储到不同的节点中。此外还可以和功能拆分联合使用，有时候会有非常好的场景。
原则上，全局数据一般存储在单一节点上，并通常保存在类似memcached缓存中，而只针对需要的数据进行分片，这里需要的数据指将会增长得非常庞大的数据（比如博客服务的用户数据）。同时分片对原因系统架构会有较大影响，除非在规划之初已设想到后期的会分片。对于分片的增加的成本，建议是：如非必要，尽量不要分片，先通过设计MySQL版本或者系统优化来延迟分片的到来。
...
生成全局唯一ID
分片工具
Hibernate Shards是一个支持分片的数据库抽象层，基于Java语言开源可用的。
Sphinx是一个全文检索工具，对于存储的查询有帮助。

总结
MySQL的扩展是应对数据增长不得不考虑的事情，数据分片、负载均衡等策略看起来很美好，但最好的策略是实现应用所明确的需求，并为可能的快速增长做好预先规划，成功的规划是可以为任何必要的措施筹集各类资源以满足需求。
